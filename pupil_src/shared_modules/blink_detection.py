...
class Offline_Blink_Detection(Observable, Blink_Detection):
    ...
    def export(self, export_window, export_dir):
        ...
        with open(
            ...
            np.savetxt('activity.csv', self.activity, delimiter=',')
            np.savetxt('filter_response.csv', self.filter_response, delimiter=',')
            merged = np.column_stack((self.timestamps, self.filter_response))
            print('merged', merged.shape)
            np.savetxt('timestamps_fitlerrespone.csv', merged, fmt='%10.6f', delimiter=',')
    ...
    def recalculate(self):
        ...
        self.activity = np.fromiter(conf_iter, dtype=float, count=len(all_pp))
        ...
        self.filter_response = fftconvolve(self.activity, blink_filter, "same") / 0.45

        print('self.filter_response', self.filter_response)
        logger.info(self.filter_response)
        print('===========================')
        print('activity', type(self.activity))
        print('activity_shape', self.activity.shape)
        print('blink_filter', type(blink_filter))
        print('type', type(self.filter_response))
        print('len', len(self.filter_response))
        print('shape', self.filter_response.shape)
        print('element_type', type(self.filter_response[0]))
